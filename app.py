# ============ ID SWIFT - VERCEL DEPLOYMENT READY ============
import os
import json
import uuid
...
# Since Vercel doesn't allow file writes, we use in-memory storage
# For production, connect to Supabase or MongoDB
...
def favicon():
    """Return empty response for favicon to avoid 500 errors"""
    return '', 204
...
def delete_holder(holder_id):
    ...
    try:
        global holders_db
        holders_db = [h for h in holders_db if h['id'] != holder_id]
...
def get_dashboard_analytics():
    ...
    try:
        ...
        dept_counts = Counter([h.get('department', 'Unassigned') for h in holders_db])
        template_counts = Counter([h.get('template', 'corporate') for h in holders_db])
        
        return jsonify({
            'summary': {
                'total_cards': len(holders_db),
                'total_departments': len(dept_counts),
                'total_templates': 4,
                'active_rate': 100 if holders_db else 0
            },
            'charts': {
                'department_distribution': [{'name': k, 'count': v} for k, v in dept_counts.items()],
                'template_usage': [{
                    'name': TEMPLATES.get(k, {}).get('name', k), 
                    'count': v, 
                    'color': TEMPLATES.get(k, {}).get('primary_color', '#000')
            ...
            'recent_activity': [
                ...
                } for h in holders_db[-10:]
            ],
            'growth_metrics': {
                'this_month': len(holders_db),
                'last_month': 0,
                'growth_percentage': 0
            }
        })
...
def generate_card(holder_id):
    ...
    try:
        ...
        holder = next((h for h in holders_db if h['id'] == holder_id), None)
        ...
        # Font handling - use default if custom fails
        try:
...
def bulk_import():
    """Bulk import card holders from CSV"""
    try:
        data = request.get_json()
...
def health_check():
    ...
    return jsonify({
        ...
        'cards_count': len(holders_db)
    })

# ============ HEALTH CHECK FOR VERCEL ============

# This is required for Vercel - the 'app' object must be at module level
